...
const UpdateArticle = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const resArticle = await Apis.get(
        endpoints["article-id"](param.get("id"))
      );
      setArticle((item) => {
        return {
          title: resArticle.data.title,
          category: resArticle.data.categoryID,
          description: resArticle.data.description,
          content: resArticle.data.content,
        };
      });
      setImage(resArticle.data.image);
  ...
  const submit = (e) => {
    ...
    const handle = async () => {
      ...
      const res = await authApi().post(
        endpoints["update-article"](param.get("id")),
        form
      );
  ...
  return (
    <form onSubmit={(e) => submit(e)} className="w-[80%] mx-auto mt-24">
      <div className="text-3xl font-semibold my-5">Chỉnh sửa bài viết</div>
      <div className="border border-slate-300 rounded-md px-5 py-3">
        ...
        <div className="">
          ...
          <input
            ...
            name="image"
            className="border p-2 w-full cursor-pointer mt-2 text-slate-500 rounded-lg"
          />
